...
Проект не содержит пользовательского frontend-а: нет HTML/UI surface, Vite/Tailwind
toolchain и браузерных assets. HTTP surface намеренно ограничен служебным
endpoint-ом `/metrics`.
...
- Машина состояний: `new`, `waiting`, `assignment_requested`, `operator_dialing`, `connected`, финальные missed-статусы.
