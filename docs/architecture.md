...
- События сброса от Telephony/Kafka маппятся в `MarkCallHungUpFromKafkaCommand` по `external_call_id`. Если call ещё `new`, `waiting`, `assignment_requested` или `operator_dialing`, финальный статус выбирается правилом: `missed`, `callback_missed`, `hangup_on_retry`.
...
- `assignment_requested/operator_dialing -> connected` по событию установленного соединения; при переходе Calls очищает локальную `operators.reserved_call_id`;
...
|---|---|---|---|---|
| Telephony | внешний/гипотетический | Исполняет команды назначения, звонит оператору, соединяет клиента и оператора, сообщает факты | Kafka facts: `operator_dialing`, `bridge_established`, `operator_no_answer`, `operator_leg_dropped`, `hangup` | Не выбирает оператора и не хранит business lifecycle Calls |
